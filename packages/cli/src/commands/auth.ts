...
export async function authCommand(args: string[]): Promise<void> {
  ...
  switch (sub) {
    case 'login': {
      ...
      const styleArg = parseFlag(rest, '--style') as 'query' | 'bearer' | 'token' | 'header' | null;
      const headerArg = parseFlag(rest, '--header');
      ...
      await auth.login(token.trim(), baseArg || undefined, styleArg || undefined, headerArg || undefined);
