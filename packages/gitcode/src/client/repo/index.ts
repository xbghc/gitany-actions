...
export async function getRepoSettings(client: GitcodeClient, owner: string, repo: string): Promise<RepoSettings> {
...
export async function getRepoEvents(client: GitcodeClient, owner: string, repo: string): Promise<RepoEvents> {
  const url = repoEventsUrl(owner, repo);
  const data = await client.request<unknown>(url, 'GET', {});
  return repoEventsSchema.parse(data);
}

export async function getContributors(client: GitcodeClient, owner: string, repo: string): Promise<Contributors> {
  const url = contributorsUrl(owner, repo);
  const data = await client.request<unknown>(url, 'GET', {});
  return contributorsSchema.parse(data);
}

export async function getBranches(client: GitcodeClient, owner: string, repo: string): Promise<Branches> {
  const url = branchesUrl(owner, repo);
  const data = await client.request<unknown>(url, 'GET', {});
  return branchSchema.array().parse(data);
}

export async function getBranch(client: GitcodeClient, owner: string, repo: string, branch: string): Promise<Branch> {
  const url = branchUrl(owner, repo, branch);
  const data = await client.request<unknown>(url, 'GET', {});
  return branchSchema.parse(data);
}

export async function getCommits(client: GitcodeClient, owner: string, repo: string): Promise<Commits> {
  const url = commitsUrl(owner, repo);
  const data = await client.request<unknown>(url, 'GET', {});
  return commitSchema.array().parse(data);
}

export async function getFileBlob(client: GitcodeClient, owner: string, repo: string, sha: string): Promise<FileBlob> {
  const url = fileBlobUrl(owner, repo, sha);
  const data = await client.request<unknown>(url, 'GET', {});
  return fileBlobSchema.parse(data);
}

export async function compare(client: GitcodeClient, owner: string, repo: string, base: string, head: string): Promise<Compare> {
  const url = compareUrl(owner, repo, base, head);
  const data = await client.request<unknown>(url, 'GET', {});
  return compareSchema.parse(data);
}

export async function getWebhooks(client: GitcodeClient, owner: string, repo: string): Promise<Webhooks> {
  const url = webhooksUrl(owner, repo);
  const data = await client.request<unknown>(url, 'GET', {});
  return webhookSchema.array().parse(data);
}

export async function getWebhook(client: GitcodeClient, owner: string, repo: string, id: number): Promise<Webhook> {
